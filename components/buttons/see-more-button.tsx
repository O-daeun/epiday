...
export default function SeeMoreButton({ onClick, disabled, isLoading, children }: Props) {
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className="mx-auto mt-10 flex items-center gap-1 rounded-full border border-var-line-200 px-[18px] py-3 duration-100 hover:shadow-custom sm:mt-20 sm:gap-2 sm:px-10"
    >
      {isLoading ? (
